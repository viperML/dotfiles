...
local widget_ip = wibox.widget({
	...
	{
		awful.widget.watch(
			[[ sh -c "sleep 15 && ip -4 a | sed -n '/global/p' | awk '{print $2}' | cut -d'/' -f1" ]],
			1800,
...
local widget_fs = awful.widget.watch([[ sh -c "$DOTDIR/bin/disks.py" ]], 1800, function(widget, stdout)
...
function bind_tag(tag_i, keysim)
	globalkeys = gears.table.join(
		globalkeys,
		-- View tag only.
		awful.key({ modkey }, keysim, function()
			local screen = awful.screen.focused()
			local tag = tags[tag_i]
			if tag then
				sharedtags.viewonly(tag, screen)
			end
		end, { description = "view tag #" .. tag_i, group = "tag" }),
		-- Toggle tag display.
		awful.key({ modkey, "Control" }, keysim, function()
			local screen = awful.screen.focused()
			local tag = tags[tag_i]
			if tag then
				sharedtags.viewtoggle(tag, screen)
			end
		end, { description = "toggle tag #" .. tag_i, group = "tag" }),
		-- Move client to tag.
		awful.key({ modkey, "Shift" }, keysim, function()
			if client.focus then
				local tag = tags[tag_i]
				if tag then
		...
		end, { description = "move focused client to tag #" .. tag_i, group = "tag" }),
		-- Toggle tag on focused client.
		awful.key({ modkey, "Control", "Shift" }, keysim, function()
			if client.focus then
				local tag = tags[tag_i]
				if tag then
		...
		end, { description = "toggle focused client on tag #" .. tag_i, group = "tag" })
	)
end
...
awful.rules.rules = {
	...
	{ rule = { class = "Firefox Beta" }, properties = { tag = tags[1], switch_to_tags = true } },
	{ rule = { class = "Code" }, properties = { tag = tags[2], switch_to_tags = true } },
	{ rule = { class = "Emacs" }, properties = { tag = tags[2], switch_to_tags = true } },
	{ rule = { class = "discord" }, properties = { tag = tags[9] } },
	{ rule = { class = "Spotify" }, properties = { tag = tags[10] } },
	{ rule_any = { class = { "Lutris", "Steam" } }, properties = { tag = tags[8] } },
	{ rule = { class = "Thunderbird" }, properties = { tag = tags[6], sticky = true } },
	{ rule = { class = "Virt-manager" }, properties = { tag = tags[7] } },
	{ rule = { class = "looking-glass-client" }, properties = { tag = tags[7] } },

	-- Fix st using even/odd proportions
	{ rule = { class = "st-256color" }, properties = { size_hints_honor = true } },
}
-- }}}
